...
col_frame['count'] = col_frame['color_rgb'].apply(lambda x: count_occurences_color(x, flattened_cluster))
...
col_frame['color_matched'] = col_frame['color_rgb'].apply(lambda x: closest_color(citadel_paint_colors, x))
# add color names from a df...
col_frame['Paint'] = col_frame['color_matched'].apply(lambda x: return_paint_name(x, citadel_paints))
col_frame_shortened = col_frame.groupby(by='Paint').agg(
    {'percent':'sum','rgb_plt':'first'}).sort_values(
